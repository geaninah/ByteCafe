...
module.exports = function(req, res, next) {
  ...
  if(!req.isAuthenticated() && typeof req.cookies.rememberme != "undefined") {
    ...
    query = "select * from remember_me_tokens where token_selector = ?";
    params = [selector];
    ...
    GLOBAL.connection.query(query, params, function(err, rows){
      ...
      if (validator == rows[0].token_validator) {
        // load the user from the database
        connection.query("select * from users where users.user_id = ?", [rows[0].token_user_id], function(err, users) {
          var user = users[0];
          // log the user in
          req.login(user, function(err) {
            // fail on error
            if (err) return next(err);
            console.log("transparently logged in:" + user.email);
            return next();
          });
        });
